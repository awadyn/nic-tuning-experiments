...
df_comb_400k = df_comb[df_comb['target_QPS'] == 400000].copy()
df_comb_600k = df_comb[df_comb['target_QPS'] == 600000].copy()
...
edp_fig_200k = px.scatter(df_comb_200k, 
                     ...
                     custom_data=['i', 'itr', 'rapl', 'dvfs', 'sys', 'target_QPS', 'sys'],
                     title=f'Memcached 200K QPS')

edp_fig_400k = px.scatter(df_comb_400k, 
                     x='time', 
                     y='joules', 
                     color='sys',
                     labels={'time': 'Time (s)', 'joules': 'Energy (Joules)'}, 
...
edp_fig_600k = px.scatter(df_comb_600k, 
                     x='time', 
                     y='joules', 
                     color='sys',
                     labels={'time': 'Time (s)', 'joules': 'Energy (Joules)'}, 
...
def update_timeline_plots1(clickData):
    ...
    num, itr, rapl, dvfs, sys, target_QPS, sys = custom_data
    return int(num), int(itr), int(rapl), dvfs, int(target_QPS), 0, sys
...
@app.callback(
    ...
    Output('core-dd', 'value'),
    Output('sys-dd', 'value'),
    [Input('edp-scatter-400k', 'clickData')]
)
def update_timeline_plots2(clickData):
    print(clickData)
    custom_data = clickData['points'][0]['customdata']
    num, itr, rapl, dvfs, sys, target_QPS, sys = custom_data
    return int(num), int(itr), int(rapl), dvfs, int(target_QPS), 0, sys
...
if __name__=='__main__':
    app.run_server(host='10.241.31.7', port='8030')
